		...
		while(exist){
			...
			if((bitmap[temp_bitmap_index] & temp_mask1) == 0){
				...
				//unsigned int begin_bitmap_index = (begin_frame - base_frame_no)/4;
					for(int j = begin_frame; j < begin_frame + _n_frames; j++){
						unsigned int index = (j - base_frame_no) / 4;
						unsigned char mask = 0x80 >> (j - base_frame_no) % 4;
						bitmap[index] ^= mask;		
					}			
					unsigned int begin_index = (begin_frame - base_frame_no) / 4;
					unsigned char temp_mask2 = 0x08>>((begin_frame-base_frame_no)%4);
...
void ContFramePool::release_frames(unsigned long _first_frame_no)
{
	...
	//we see if the first frame is the head of sequence
	...
	if((cur_pool.bitmap[bitmap_index] & mask1) == 0){
...
//	unsigned char mask = 0x80 >> ((_first_frame_no - cur_pool.base_frame_no) % 4);
	...
	for(i = _first_frame_no; i < cur_pool.base_frame_no + cur_pool.nframes; i++){
		...
		if((0x80 >> (frame_diff % 4)) & cur_pool.bitmap[bitmap_index] == 0) break;
