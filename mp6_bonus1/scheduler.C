...
*/
	add(_thread);
//--------------------------begin of mirror disk-----------------------------

	
//	Console::puts("\nthe # of m_disk_counter is\n");
//	Console::putui(m_disk_counter);
//	for(;;);

	if(m_disk_counter != 0){
		

		MirroredDisk* mirrored_disk = m_disk[0];
//		Console::puts("\nThere are  mirrored disk \n");
//		Console::puti(m_disk_counter);
//		Console::puts("\nThis should not be seen\n");
//		for(;;);
//		unsigned int cc = mirrored_disk->mirrored_thread_count;
//		(*mirrored_disk).increment_thread_count();
//		Console::puts("\nwe get cc\n");
//		Console::putui(mirrored_disk->mirrored_thread_count);
//		Console::puts("\nwe get cc\n");
//		Console::putui(mirrored_disk->mirrored_block_count());

//		for(;;);
		if(mirrored_disk->mirrored_block_count() > 0){

//			Console::puts("\nThis should not be seen1111\n");
//			for(;;);
			mirrored_node* temp_node = mirrored_disk->front();
		//if there is no blocked thread at all
			for(int i  = 0; i < mirrored_disk->mirrored_block_count(); i++){
...
//				Console::putui(Machine::inportb(0x1F7));
				...
				if(Machine::inportb(0x177) & 0x08){
					mirrored_disk->unblock(temp_node->thread);
					add(temp_node->thread);	
//					if(temp_node = NULL)	return;
					Console::puts("there is blocked thread inside");
					for(;;);		
//				node* newNode = (*thread_list_head).next;	
...
//		Console::puts("\nthread 3 did not detect the ready port\n");
	}


//--------------------------------this is the division of two disks----------
...
void Scheduler::register_mirrored_disk(MirroredDisk* mirrored_disk){
	if(m_disk_counter == 0){
		m_disk[0] = mirrored_disk;
		m_disk_counter += 1;
	}else{
		m_disk[1] = mirrored_disk;
		m_disk_counter += 1;
	}
}
