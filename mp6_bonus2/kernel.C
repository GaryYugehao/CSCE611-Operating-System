...
//-----------------this is where we begin to deal with exception 14------

    class Concurrency_Handler: public InterruptHandler {
      public:
      virtual void handle_interrupt(REGS * _regs) {
//        Console::puts("interrupt 14 is handled\n");
...
//	if(SYSTEM_SCHEDULER->disk_counter != 0){


		BlockingDisk* blocking_disk = SYSTEM_SCHEDULER->disk[0];
...
//			for(int i  = 0; i < blocking_disk->block_count(); i++){
				...
				if(temp_node == NULL){
      				 Console::puts("\tthis is in kernel.C, temp_node == nulln\n");
					for(;;);
				}
		
//				if(Machine::inportb(0x1F7) & 0x08){
					blocking_disk->unblock(temp_node->thread);
					SYSTEM_SCHEDULER->add(temp_node->thread);	
//					Console::puts("\ntotal thread in the ready queue\n");
...
//---------------------------end---------------------
    ...
    InterruptHandler::register_handler(14,  &concurrency_handler);

//	Console::puts("\nthis is where we register exc_no 14\n");
//	for(;;);

//-----------------thsi is where we end dealing with exception 14--------
