...
//	Console::puts("\nwe got file ");
//	Console::putui(file1->FileId());
//	Console::puts("\nwe got file ");
//	Console::putui(file2->FileId());
//	for(;;);
...
/*
	unsigned int block = FILE_SYSTEM->free_block;
	unsigned char buf[512];
	Console::puts("\nafter first exercise file system");
	for(int k = 0; k < 10; k++){
		Console::puts("\n k = ");
		Console::putui(k);
		Console::puts(", free_block =");
		Console::putui(block);
		FILE_SYSTEM->disk->read(block, buf);
		memcpy(&block, buf+508, 4);
